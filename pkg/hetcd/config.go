...
func (c *Config) Equal(other *Config) bool {
	...
	return hutl.Equal(c.AutoSyncInterval, other.AutoSyncInterval) &&
		hutl.Equal(c.DialTimeout, other.DialTimeout) &&
		hutl.Equal(c.DialKeepAliveTime, other.DialKeepAliveTime) &&
		hutl.Equal(c.DialKeepAliveTimeout, other.DialKeepAliveTimeout) &&
		hutl.Equal(c.MaxCallSendMsgSize, other.MaxCallSendMsgSize) &&
		hutl.Equal(c.MaxCallRecvMsgSize, other.MaxCallRecvMsgSize) &&
		hutl.Equal(c.Username, other.Username) &&
		hutl.Equal(c.Password, other.Password) &&
		hutl.Equal(c.RejectOldCluster, other.RejectOldCluster) &&
		hutl.Equal(c.PermitWithoutStream, other.PermitWithoutStream) &&
		hutl.Equal(c.MaxUnaryRetries, other.MaxUnaryRetries) &&
		hutl.Equal(c.BackoffWaitBetween, other.BackoffWaitBetween) &&
		hutl.Equal(c.BackoffJitterFraction, other.BackoffJitterFraction)
}
