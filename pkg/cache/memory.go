...
func NewMinExpire(duration time.Duration) Option {
	return &minExpire{value: int64(duration / time.Millisecond)}
}

type minExpire struct {
	value int64
}

func (m *minExpire) getValue() any {
	return m.value
}

// NewMaxExpire 参数最大有效时间
func NewMaxExpire(duration time.Duration) Option {
	return &maxExpire{value: int64(duration / time.Millisecond)}
}

type maxExpire struct {
	value int64
}

func (m *maxExpire) getValue() any {
	return m.value
}
...
type readData[K utl.SliceType, V any] func(ctx context.Context, key K) (*V, error)

// item 缓存Item
type item[K utl.SliceType, V any] struct {
	lock      sync.RWMutex
...
func NewMemoryCache[K utl.SliceType, V any](option ...Option) *MemoryCache[K, V] {
	ret := &MemoryCache[K, V]{
	...
	for _, item := range option {
		if val, ok := item.(*minExpire); ok {
			ret.minExpire = val.value
		} else if val, ok := item.(*maxExpire); ok {
			ret.maxExpire = val.value
		}
	}
...
// MemoryCache 内存缓存
type MemoryCache[K utl.SliceType, V any] struct {
	lock      sync.RWMutex
	maps      map[K]*item[K, V]
	call      readData[K, V]
	minExpire int64 //最小有效期（毫秒），默认 5分钟
...
func defaultReadCall[K utl.SliceType, V any](ctx context.Context, key K) (*V, error) {
	return nil, nil
}
