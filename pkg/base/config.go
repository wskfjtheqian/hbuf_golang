...
func (con *Config) CheckConfig() int {
	...
	if nil == con.Redis {
		errCount++
		glog.Errorln("未找到Redis的配置文件")
	} else {
		errCount += con.Redis.CheckConfig()
	}
	if nil == con.DB {
		errCount++
		glog.Errorln("未找到数据库的配置文件")
	} else {
	...
	if 0 == con.DataCenterId {
		errCount++
		glog.Errorln("机房ID设置错误，请设置 data_center_id 大于 0")
	}

	if 0 == con.WorkerId {
		errCount++
		glog.Errorln("机器ID设置错误，请设置 worker_id 大于 0")
	}
	return errCount
}
